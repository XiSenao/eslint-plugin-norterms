...
module.exports = {
  ...
  create: function (context) {
    ...
    const { forceUpdateConfig = false, maxCacheTime = MAX_CACHE_TIME } = options || {};
    if (!isFetchConfig) {
      isFetchConfig = true;
      /** 检测配置文件修改时间 */
      fs.stat(`${__dirname}/../../config/config.js`, (err, val) => {
        if (err) {
          console.log(`${chalk.red.bold('配置文件版本信息获取失败')}`);
          return;
        }
        const { atime } = val || {};
        ...
        if (new Date().getTime() - fileAccessTime > maxCacheTime || forceUpdateConfig) {
          instance.get('https://gitee.com/Xashen/eslint-plugin-norterms/raw/master/config')
            .then(configSource => {
              try {
                // const configCode = JSON.stringify(configSource).replace(/(\\r|\\n)\s*/g, '');
                fs.writeFile(`${__dirname}/../../config/config.js`, `module.exports = ${JSON.stringify(configSource.data)}`, (error) => {
                  if (error) {
                    console.log(`\n${chalk.red.bold('配置文件更新失败')}, ${chalk.bgCyan.bold('原因如下:')}`);
                ...
                console.log(`${chalk.red.bold('解析配置文件失败')}`);
              }
            })
            .catch(_ => {
              console.log(`${chalk.red.bold('获取配置文件失败')}`, _);
            });
        }
      })
    }
